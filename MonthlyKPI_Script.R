...
setwd("U:/WorkSpaces/fkm_data/Program Team KPIs/")
 Social_Logins <- read_csv("Raw/Users,_last_login,_social_studio-2021-12-01.csv")
 MC_Logins <- read_csv("Raw/Users,_last_login,_marketing_cloud-2021-12-01.csv")
...
library(readxl)
perms2prods <- read.csv("Raw/user_perms_prods.csv")
View(perms2prods)

#users<-read.csv("Raw/MtD Licensed Users Logged In-2021-07-30-17-00-08.csv")
...
users<-subset(user_logins, month(user_logins$LoginTime)== last_month & year(user_logins$LoginTime)==this_year)
...
names(perms2prods)
perms2prods<-perms2prods[,c(2,17)]
...
names(users_perms_prods)
upp_clean <- users_perms_prods[, c(1,2,4,7,8,11:13)]
...
names(MC_s_c_u)
colnames(MC_s_c_u)[16]<-"SF_LastLoginDate"

MC_s_c_u <- MC_s_c_u[, -c(4,6,15,18,7)]
...
length(unique(df_foruse$Email.x[df_foruse$MCProfile=="MC"]))

#restrict columns to those we need
names(df_foruse)
df_fortable <- df_foruse[, -c(1,2,4,6,8,10,11,12,13,15,16)]
...
write.csv(user_counts_by_product, paste0("U:/WorkSpaces/fkm_data/Program Team KPIs/user_counts_by_product", last_month, this_year, ".csv"), row.names = FALSE)
write.csv(user_cts_by_prd_unit, paste0("U:/WorkSpaces/fkm_data/Program Team KPIs/user_cts_by_prd_unit", last_month, this_year, ".csv"), row.names = FALSE)
write.csv(users_by_unit, paste0("U:/WorkSpaces/fkm_data/Program Team KPIs/users_by_unit", last_month, this_year, ".csv"), row.names = FALSE)
write.csv(prds_by_unit, paste0("U:/WorkSpaces/fkm_data/Program Team KPIs/prds_by_unit", last_month, this_year, ".csv"), row.names = FALSE)
write.csv(prds_by_unit_reshaped, paste0("U:/WorkSpaces/fkm_data/Program Team KPIs/prds_by_unit_reshaped", last_month, this_year, ".csv"), row.names = FALSE)
...
units_max_min <- units_by_month %>%
    ...
    arrange(Parent_Organization__c)
...
write.csv(unit_FirstTrellisMonth, "U:/WorkSpaces/fkm_data/Program Team KPIs/Usage and Adoption/unit_FirstTrellisMonth_062021.csv")
...
logins<-merge(users, user_logins, by.x = "Id", by.y = "UserId", all.x = TRUE)
...
logins_firstlogin <- logins %>%
    ...
    summarise(
        MinLoginDate = min(LoginTime, na.rm = T)
    ) %>%
    arrange(Email, CreatedDate, Profile.Name, UserRole.Name)

###stopped here 19 April 2021
logins_firstlogin$T2firstlogin<-difftime(as.Date(logins_firstlogin$MinLoginDate), as.Date(logins_firstlogin$CreatedDate), units = c("days"))
