...
df_fortable <- filter(df_foruse2, !Profile.Name %in% target)  # equivalently, dat %>% filter(name %in% target)
df_fortable %>% 
  distinct()
...
expand_grid(mean=1:3, sd=1:3) %>% 
  rowwise() %>% 
  mutate(data = list(rnorm(100, mean = mean, sd = sd)))
unnest() %>% 
  ggplot(aes(x = data)) +
  geom_histogram() +
  facet_grid(rows = vars(mean), cols = vars(sd))

# 5 rows from every category
storms %>% 
  group_by(category) %>% 
  slice_sample(n=5)

# samples of whole groups
storms %>% 
  group_by(category) %>% 
  nest() %>% 
  ungroup() %>% 
